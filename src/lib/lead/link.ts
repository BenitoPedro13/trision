...
export interface DadosContatoWhatsapp {
 ...
 *
 * Message shape follows §7.3 exactly, minus the trailing `[codigo]` until `/ir/` exists. */
export function montarLinkWhatsapp(dados: DadosContatoWhatsapp): string | null {
  ...
  let mensagem = `Olá! ${origem}${dados.produtoNome} — ${ficha}.`;
  if (dados.codigo) mensagem += `  [${dados.codigo}]`;
